...
class NotificationService
{
    ...
    {
        return match ($type) {
            NotificationType::CUSTOMER_SIGNUP => sprintf('Bienvenue chez %s sur Lakarte', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::EQUIPIER_ASSIGNED => sprintf('Votre espace équipier est actif chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::EQUIPIER_REMOVED => sprintf('Votre accès équipier a été désactivé chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CARD_CREATED => sprintf('Votre carte fidélité est prêt chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CARD_COMPLETED => sprintf('Votre récompense est prêt chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::POINTS_ADDED => sprintf('Votre carte a été mise à jour chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::REWARD_CLAIMED => sprintf('Récompense récupérée chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::PROMOTIONAL_OFFER_STARTS => sprintf('Nouveau bon plan disponible chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::PROMOTIONAL_OFFER_ENDING_SOON => sprintf('Plus que 2 jours pour profiter du bon plan chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::PROMOTIONAL_OFFER_FLASH_DAY_BEFORE => sprintf('Demain chez %s : offre flash à ne pas manquer !', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::PROMOTIONAL_OFFER_FLASH_DAY_OF => sprintf("Aujourd'hui seulement chez %s : offre flash !", $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CONTEST_DAY_BEFORE_START => sprintf('Demain, nouveau jeu concours chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CONTEST_STARTS => sprintf('Le jeu concours commence aujourd\'hui chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CONTEST_ENDING_SOON => sprintf('Plus que 2 jours pour participer au jeu concours chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CONTEST_DRAW_DAY => sprintf('Tirage aujourd\'hui chez %s : bonne chance !', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CONTEST_PARTICIPATION_UPDATED => sprintf('Votre participation au jeu concours chez %s', $merchant->getCompanyName() ?? 'Lakarte'),
            NotificationType::CONTEST_WINNER => sprintf('🎉 Bravo, vous avez gagné au jeu concours chez %s !', $merchant->getCompanyName() ?? 'Lakarte'),
            default => null,
        };
    }
